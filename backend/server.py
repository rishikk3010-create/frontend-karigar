...
async def _build_worker_doc(payload: WorkerProfilePayload, phone: str, user_id: Optional[str]) -> dict:
    ...
    return {
        ...
        "portfolio_images": payload.portfolio_images or [],
        "aadhar_images": payload.aadhar_images or [],
        "employment_proof_type": payload.employment_proof_type,
        "employment_proof_images": payload.employment_proof_images or [],
        "referral_code": code,
...
_SNAPSHOT_FIELDS = [
    "full_name", "dob", "gender", "languages", "area", "city", "skills",
...
async def create_my_profile(payload: WorkerProfilePayload, user: dict = Depends(get_current_user)):
    ...
    doc = await _build_worker_doc(payload, user["phone"], user["id"])
...
async def update_my_profile(payload: WorkerProfilePayload, user: dict = Depends(get_current_user)):
    ...
    if not worker:
        raise HTTPException(status_code=404, detail="Profile not found")
    update = _profile_update_fields(payload, worker)
...
async def admin_search_workers(
    ...
    items = [clean(w) for w in await cursor.to_list(page_size)]
...
async def admin_update_worker(worker_id: str, payload: WorkerProfilePayload, user: dict = Depends(require_roles(*ADMIN_ROLES))):
    ...
    return clean(await db.workers.find_one({"id": worker_id}))
...
async def admin_register_worker(payload: AdminRegisterWorkerPayload, user: dict = Depends(require_roles(*ADMIN_ROLES))):
    ...
    if await db.workers.find_one({"phone": phone}):
        raise HTTPException(status_code=400, detail="A worker with this mobile already exists")
    base = WorkerProfilePayload(**payload.dict(exclude={"mobile"}))
    doc = await _build_worker_doc(base, phone, None)
    await db.workers.insert_one(dict(doc))
    await _register_referral(doc)
    return clean(doc)
...
async def verification_queue(user: dict = Depends(require_roles(*VERIFY_ROLES))):
    ...
    return [clean(w) for w in await cursor.to_list(200)]
...
async def approve_worker(worker_id: str, background_tasks: BackgroundTasks, user: dict = Depends(require_roles(*VERIFY_ROLES))):
    ...
    background_tasks.add_task(email_service.send_profile_email, clean(updated), referred_by)
    return clean(updated)
